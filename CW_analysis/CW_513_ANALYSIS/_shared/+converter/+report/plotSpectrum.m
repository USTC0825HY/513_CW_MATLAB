...
function label = localFormatFrequency(frequencyHz)
% Select a readable engineering unit without rounding small values to zero.
if frequencyHz >= 1e6
    label = sprintf('%.6g MHz', frequencyHz / 1e6);
elseif frequencyHz >= 1e3
    label = sprintf('%.6g kHz', frequencyHz / 1e3);
else
    label = sprintf('%.6g Hz', frequencyHz);
end
end
