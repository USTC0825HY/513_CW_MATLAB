...
两条输入链路的前级 R 不同，带宽按通道做源阻抗去嵌：

- **IN1（X10）**：OP27 差分缓冲驱动，源阻抗≈0，R=板级 33Ω，**系数=1，不做修正**；
- **IN2–IN5（X11-1/2/3/4/5）**：信号源直连，SDG6000 源内阻 50Ω 与板级 33Ω（R318–R322）串联，R=83Ω；单极点 RC 去嵌为 **频率轴与 −3dB 带宽 ×(33+50)/33≈2.5151**，图题与横轴注明系数。

系数规则在 `private/adc128Config.m`（`bandwidthScaleRules`，按文件名匹配），`analysis_parameters.csv` 记录 `bandwidth_scale_factor/reason`。交叉验证：X11 实测 12.18–12.48 kHz ×2.5151 ≈ 30.6–31.4 kHz，与 X10 实测 31.07 kHz（同为板级 33Ω+150nF）吻合。
