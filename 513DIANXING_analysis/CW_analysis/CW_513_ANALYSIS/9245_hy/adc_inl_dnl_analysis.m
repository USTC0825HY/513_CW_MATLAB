function results = adc_inl_dnl_analysis(dataFolder, selectedFiles, outputFolder, runOptions)
%ADC_INL_DNL_ANALYSIS Run the fixed AD9245 INL/DNL analysis.
%   RESULTS = ADC_INL_DNL_ANALYSIS() prompts for a data directory and CSV
%   files, then writes a new timestamped result directory.

bootstrapRuntime();
if nargin < 1, dataFolder = []; end
if nargin < 2, selectedFiles = []; end
if nargin < 3, outputFolder = []; end
if nargin < 4, runOptions = struct(); end
[config, ~] = converter.runtime.applyRunOptions(ad9245Config('inl_dnl'), runOptions);
config.allowRadixPrompt = isempty(selectedFiles);
[config, selectedFiles, dataFolder, radixCancelled] = converter.io.prepareAdcRadix( ...
    config, dataFolder, selectedFiles);
if radixCancelled, results = []; return; end
results = converter.adc.runInlDnl(config, ...
    dataFolder, selectedFiles, outputFolder);
end
