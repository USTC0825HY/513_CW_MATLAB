function result = run_da9726_batch(dataRoot, scaleFolder, pairManifest)
%RUN_DA9726_BATCH Run all supported DA9726 analyses with explicit roots.
if nargin < 1 || isempty(dataRoot), error('cw513:DataRootRequired', ...
        '批处理必须显式提供DA9726数据根目录。'); end
result = struct();
result.noise = dac_noise_analysis(fullfile(dataRoot, '03_Noise'), {}, '');
