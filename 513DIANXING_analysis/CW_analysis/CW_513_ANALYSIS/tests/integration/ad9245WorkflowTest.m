classdef ad9245WorkflowTest < matlab.unittest.TestCase
    ...
    methods (Test)
        function runsSfdrEntry(testCase)
            ...
            testCase.verifyEqual(results.SampleStride, 5);
            testCase.verifyEqual(results.SourceSampleRateHz, 25e6, ...
                AbsTol=1e-9);
            testCase.verifyEqual(results.AnalysisSampleRateHz, 5e6, ...
                AbsTol=1e-9);
            testCase.verifyEqual(results.ResultUse, ...
                "旧25 MHz ILA数据抽样估算");
        ...
        function runsSfdrWithoutDecimationForSynchronousCapture(testCase)
            ...
            options = struct('configOverride', struct( ...
                ...
                'sfdrSampleStride', 1, ...
                'sfdrAnalysisSampleRateHz', 20e6, ...
                'sfdrSamplingMode', 'synchronous_20mhz', ...
                'sfdrResultUse', '20 MHz同步采集'));
            results = adc_sfdr_analysis(testCase.DataFolders.sfdr, files, ...
                testCase.OutputFolders.sfdr, options);
            testCase.verifyEqual(results.SourceSampleCount, 8192);
            testCase.verifyEqual(results.AnalysisSampleCount, 8192);
            testCase.verifyEqual(results.SampleStride, 1);
            testCase.verifyEqual(results.AnalysisSampleRateHz, 20e6, ...
                AbsTol=1e-9);
        end
