...
%RUN_GOLDEN_REGRESSION Compare five real X3G workflows with tagged output.
...
specs(3) = createSpec('power_scale', fullfile(dataRoot, ...
    '03_InputPowerScale', 'X3G', 'raw'), @adc_power_scale_analysis, ...
    'ADC_power_scale_summary.csv');
specs(4) = createSpec('isolation', fullfile(dataRoot, ...
    '04_Isolation', 'X3G', 'raw'), @adc_isolation_analysis, ...
    'ADC_isolation_summary.csv');
specs(5) = createSpec('inl_dnl', fullfile(dataRoot, ...
