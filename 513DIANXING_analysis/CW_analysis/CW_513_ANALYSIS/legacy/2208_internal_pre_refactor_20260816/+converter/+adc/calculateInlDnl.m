...
%   Every record receives an independent frequency, phase, amplitude, and
%   offset fit. The pooled theoretical histogram is the sample-weighted
%   mixture of those record-specific sine distributions.
...
captureStatus = cell(fileCount, 1);

for fileIndex = 1:fileCount
    adcCode = double(adcCodeList{fileIndex}(:));
    ...
    if ~isfinite(fitR2(fileIndex)) || fitR2(fileIndex) < config.minimumFitR2
        failureReasons{end + 1, 1} = 'FitR2BelowMinimum'; %#ok<AGROW>
    end
    if glitchFraction(fileIndex) > config.maximumGlitchFraction
        failureReasons{end + 1, 1} = 'ResidualGlitchDetected'; %#ok<AGROW>
    end
    if clippedSampleCount(fileIndex) > 0
        failureReasons{end + 1, 1} = 'ClippingDetected'; %#ok<AGROW>
    end
    usedForInlDnl(fileIndex) = isempty(failureReasons);
    if usedForInlDnl(fileIndex)
        captureStatus{fileIndex} = 'OK';
    else
        captureStatus{fileIndex} = strjoin(failureReasons, ';');
    end
end
...
if validCaptureCount > 0
    ...
    sharedFrequencyHz = median(refinedFrequencyHz);
end
frequencyDriftHz = refinedFrequencyHz - sharedFrequencyHz;
boundaryPhaseErrorRadians = NaN(fileCount, 1);
boundaryCodeJump = NaN(fileCount, 1);
if config.recordsAreSampleContiguous
    for fileIndex = 2:fileCount
        expectedPhaseRadians = phaseRadians(fileIndex - 1) + ...
            2*pi*sharedFrequencyHz*sampleCount(fileIndex - 1)/config.sampleRate;
        boundaryPhaseErrorRadians(fileIndex) = wrapRadians( ...
            phaseRadians(fileIndex) - expectedPhaseRadians);
        boundaryCodeJump(fileIndex) = firstCode(fileIndex) - lastCode(fileIndex - 1);
    end
end

captureTable = table(fileNames(:), sampleCount, coarseFrequencyHz, ...
...
if validCaptureCount == 0 || ...
    ...
    for fileIndex = find(usedForInlDnl).'
        ...
        recordProbability = recordProbability / probabilitySum;
        theoreticalCounts = theoreticalCounts + ...
            recordValidSampleCount * recordProbability;
    end
...
if validCaptureCount > 0
    summaryAmplitudeCode = median(amplitudeCode(usedForInlDnl));
    summaryOffsetCode = median(offsetCode(usedForInlDnl));
    summaryFitR2 = min(fitR2(usedForInlDnl));
    summaryResidualRmsCode = max(residualRmsCode(usedForInlDnl));
    summaryFrequencyLevelDb = median(frequencyLevelDb(usedForInlDnl));
else
    summaryAmplitudeCode = NaN;
    summaryOffsetCode = NaN;
    summaryFitR2 = min(fitR2);
    summaryResidualRmsCode = max(residualRmsCode);
    summaryFrequencyLevelDb = median(frequencyLevelDb);
end
...
results = table({char(channelName)}, config.sampleRate, ...
    {calibrationReference}, {formalConclusion}, {inputSha256}, ...
    fileCount, validCaptureCount, ...
    validCaptureFraction, totalSamples, validSamples, sharedFrequencyHz, ...
    std(refinedFrequencyHz(usedForInlDnl)), summaryFrequencyLevelDb, ...
    summaryAmplitudeCode, summaryOffsetCode, summaryFitR2, ...
    median(fitR2(usedForInlDnl)), summaryResidualRmsCode, ...
    sum(glitchCount), codeMin, codeMax, coverageRatio, maxAbsDnl, ...
    maxAbsInl, maxAbsInlMid, logical(config.recordsAreSampleContiguous), ...
    {statusText}, ...
...
details = struct('curveTable', curveTable, 'captureTable', captureTable, ...
    ...
    'channelName', char(channelName), 'status', statusText, ...
