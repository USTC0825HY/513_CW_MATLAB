...
frequencyMismatchVector = repmat(frequencyMismatchFlag, quietCount, 1);
pass = isolationDb > config.minimumIsolationDb & ~frequencyMismatchVector;
formalConclusion = repmat("暂不能判定", quietCount, 1);
formalConclusion(~frequencyMismatchVector & isolationDb > config.minimumIsolationDb) = "满足";
formalConclusion(~frequencyMismatchVector & isolationDb <= config.minimumIsolationDb) = "不满足";
results = table( ...
    ...
    fitR2(quietMask), fitResidualRmsCode(quietMask), pass, formalConclusion, ...
    ...
    'QuietFitR2', 'QuietResidualRmsCode', 'Pass', 'FormalConclusion'});
