...
try
    ...
    writeExcludedInputNote(runContext.folder, dataFolder);

    summary = table();
    for k = 1:numel(files)
        channel = resolveChannel(files{k});
...
function channel = resolveChannel(filePath)
name = lower(char(filePath));
if contains(name, 'jg15')
    channel = 'ADC1_JG15';
elseif contains(name, 'jg17')
    channel = 'ADC2_JG17';
elseif contains(name, 'jg22')
    channel = 'ADC5_JG22';
elseif contains(name, 'jg24')
    channel = 'ADC6_JG24';
else
    error('converter:adc:UnknownChannel', '无法从文件名识别 AD2208 通道：%s', filePath);
end
end
...
summary = table({channel}, {filePath}, {sourceSha256}, n, config.sampleRate, durationS, ...
    ...
    config.sampleRate / config.welchNfft, config.sampleRate / n, ...
...
function writeExcludedInputNote(runFolder, dataFolder)
...
fprintf(fileId, '"%s",%s\n', fullfile(dataFolder, 'JG24_noise.csv'), ...
    '用户指定 ADC6 沿用现有 1 Hz PICO 结果，本次仅重绘 ADC6 图');
end
...
fprintf(fileId, 'JG24_noise.csv is excluded by user instruction; ADC6 remains the existing 1 Hz PICO total-chain result.\n');
end
