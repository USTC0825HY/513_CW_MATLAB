...
if isfield(config, 'rejectNyquistOrAbove') && config.rejectNyquistOrAbove
    nyquistPolicy = 'excluded from -3 dB crossing (rejectNyquistOrAbove=true)';
else
    nyquistPolicy = 'retained in -3 dB crossing (default; no Nyquist filtering)';
end
parameterName = [parameterName; {'nyquist_point_policy'}];
parameterValue = [parameterValue; {nyquistPolicy}];
if isfield(config, 'bandwidthScaleFactor') && ...
        ~isempty(config.bandwidthScaleFactor)
    parameterName = [parameterName; {'bandwidth_scale_factor'}; ...
        {'bandwidth_scale_reason'}];
    parameterValue = [parameterValue; ...
        {num2str(config.bandwidthScaleFactor, 12)}; ...
        {char(string(config.bandwidthScaleReason))}];
end
