...
if strncmpi(config.deviceId, 'DA', 2)
    ...
    % the DAC code width and captured quantity separately.  Some MAT
    % analyses read the actual rate from each file, so label the configured
    % value as such instead of presenting it as the measured rate.
    if isfield(config, 'sampleRateSource') && ...
            ~isempty(config.sampleRateSource)
        fprintf(['器件：%s；分析：%s；配置采样率：%.9g Hz；来源：%s；' ...
            'DAC码宽：%d bit；采集波形：电压（PicoScope MAT）\n'], ...
            config.deviceId, config.analysisId, config.sampleRate, ...
            char(config.sampleRateSource), localDacCodeBits(config));
    else
