...
入口位于 `128_hy`，12 bit unsigned，默认CSV第4列；采样率由用户按记录时基提供。读取后只减2048，不改变CodePp。正弦/余弦/直流联合拟合给出 `CodePp=2*hypot(a,b)`；最低频3个有效点的幅值中位数作参考，响应为20log10幅值比，−3 dB在log10频率轴插值。R²≥0.99、原始码不触及≤1或≥4094且频率匹配的记录才参与。全正输入允许非零DC，但仍须避免削顶；无刻度时输出CodePp而不是Vpp。没有正式验收限值，结论为“暂不能判定”。
