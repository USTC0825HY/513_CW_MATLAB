...
```matlab
run_ad2208_batch('F:\\01_Laser\\0_20260727_513test\\CW_Data\\513_CW_DATA\\AD2208');
```

原始 CSV 只读；结果写入对应指标和通道目录的 `results`。每次运行必须先重新盘点当前数据；缺失的隔离度、噪声、时钟或其他指标会记录为“未处理”，不会生成伪结果。
