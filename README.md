...
默认路径是维护者的Windows本地路径。在其他电脑运行时，请传入本机的数据目录、文件列表、输出目录和配置。ADC刻度默认随代码携带，见 [刻度配置](CW_analysis/CW_513_ANALYSIS/CALIBRATION.md)，不需要另传工作簿。仓库不含原始测量数据、实验结果、MATLAB缓存或已删除的历史入口；保留小型测试夹具。
