...
resultMat = fullfile(cfg.outputDir, 'JG15_adc_input_equiv_new_flow.mat');
summaryCsv = fullfile(cfg.outputDir, 'JG15_adc_input_equiv_new_flow_summary.csv');
...
fprintf('Saved ASD     : %s\n', fullfile(cfg.outputDir, 'JG15_adc_input_equiv_ASD.png'));
fprintf('Saved PSD     : %s\n', fullfile(cfg.outputDir, 'JG15_adc_input_equiv_PSD.png'));
fprintf('Saved stages  : %s\n', fullfile(cfg.outputDir, 'JG15_new_flow_stage_ASD.png'));
...
fig = figure('Name', 'JG15 ADC input equivalent ASD', ...
    'Visible', visibleState, 'Color', 'w');
loglog(freqPlot, asdPlot, 'LineWidth', 1.1);
grid on;
xlabel('Frequency (Hz)');
ylabel('ADC input-equivalent ASD (nV/sqrtHz)');
title('JG15 ADC input-equivalent ASD, DAC baseline not subtracted');
...
exportgraphics(fig, fullfile(cfg.outputDir, 'JG15_adc_input_equiv_ASD.png'), ...
...
fig = figure('Name', 'JG15 ADC input equivalent PSD', ...
    'Visible', visibleState, 'Color', 'w');
loglog(freqPlot, psdPlot, 'LineWidth', 1.1);
grid on;
xlabel('Frequency (Hz)');
ylabel('ADC input-equivalent PSD (V^2/Hz)');
title('JG15 ADC input-equivalent PSD, DAC baseline not subtracted');
...
exportgraphics(fig, fullfile(cfg.outputDir, 'JG15_adc_input_equiv_PSD.png'), ...
    'Resolution', 180, 'BackgroundColor', 'white');
if ~cfg.showFigures
    close(fig);
end
...
fig = figure('Name', 'JG15 new flow stage ASD', ...
...
sgtitle('JG15 new flow ASD stages, DAC baseline not subtracted');
exportgraphics(fig, fullfile(cfg.outputDir, 'JG15_new_flow_stage_ASD.png'), ...
    'Resolution', 180, 'BackgroundColor', 'white');
if ~cfg.showFigures
    close(fig);
end
