...
% AD9245 X1G multi-level Vpp-Codepp calibration from Vivado ILA CSV files.
%
% Important evidence conventions:
%   1. CSV column 1 (Sample in Buffer) is the time coordinate.
%   2. CSV column 8 is X1G signed 14-bit code; column 9 is its valid flag.
...
%% Configuration
dataDir = 'G:\01_laser\202607_电2test\YCQD_AD_9245\9245_scale\X1G';
outDir = fullfile(dataDir, 'AD9245_X1G_scale_result');
...
for k = 1:nFile
    ...
    if numel(code) < 16
        error('Too few valid X1G samples in %s.', inPath);
    end
...
fitDefinition = repmat("source_setting_Vpp = slope * X1G_Codepp + intercept", 2, 1);
...
value = [ ...
    ...
    string(scriptVersion); "AD9245 X1G"; ...
...
measurementCsv = fullfile(outDir, 'AD9245_X1G_scale_measurements.csv');
summaryCsv = fullfile(outDir, 'AD9245_X1G_scale_fit_summary.csv');
parameterCsv = fullfile(outDir, 'analysis_parameters.csv');
manifestCsv = fullfile(outDir, 'source_manifest.csv');
matPath = fullfile(outDir, 'AD9245_X1G_scale_result.mat');
plotPath = fullfile(outDir, 'AD9245_X1G_scale_fit_residual.png');
...
grid on;
box on;
xlabel('X1G Code_{pp} (code)');
ylabel('Source setting (V_{pp})');
title(sprintf(['AD9245 X1G calibration: %.6f uV/code, ', ...
...
fprintf(fileId, 'AD9245 X1G scale calibration\n');
