...
% AD9245 X2G multi-level Vpp-Codepp calibration from Vivado ILA CSV files.
%
% Important evidence conventions:
%   1. CSV column 1 (Sample in Buffer) is the time coordinate.
%   2. CSV column 8 is X2G signed 14-bit code; column 9 is its valid flag.
...
%% Configuration
dataDir = 'G:\01_laser\202607_电2test\YCQD_AD_9245\9245_scale\X2G';
outDir = fullfile(dataDir, 'AD9245_X2G_scale_result');
...
for k = 1:nFile
    ...
    if numel(code) < 16
        error('Too few valid X2G samples in %s.', inPath);
    end
    ...
    nearRailFlag(k) = minimumCode(k) <= negativeRail + nearRailMarginCode ...
    ...
    if nearRailFlag(k)
        ...
        notes(noteCount) = 'near 14-bit code rail';
    end
...
measurementTable = table(sourceFile, sourcePath, sourceSizeBytes, ...
    ...
    nearRailFlag, primaryFitUsed, fitExclusionReason, qualityNote, ...
    primaryPredictedVpp, primaryResidualVpp, allPointPredictedVpp, ...
    allPointResidualVpp);

fitName = ["primary_non_near_rail"; "all_points_cross_check"];
fitDefinition = repmat("source_setting_Vpp = slope * X2G_Codepp + intercept", 2, 1);
...
exclusionReason = [ ...
    "near-rail points excluded by predeclared 256-code margin"; ...
    "none; cross-check only"];
...
value = [ ...
    ...
    string(scriptVersion); "AD9245 X2G"; ...
...
measurementCsv = fullfile(outDir, 'AD9245_X2G_scale_measurements.csv');
summaryCsv = fullfile(outDir, 'AD9245_X2G_scale_fit_summary.csv');
parameterCsv = fullfile(outDir, 'analysis_parameters.csv');
manifestCsv = fullfile(outDir, 'source_manifest.csv');
matPath = fullfile(outDir, 'AD9245_X2G_scale_result.mat');
plotPath = fullfile(outDir, 'AD9245_X2G_scale_fit_residual.png');
...
token = regexp(fileName, '(\d+(?:\.\d+)?)Vpp', 'tokens', 'once');
if isempty(token)
...
subplot(2, 1, 1);
hold on;
plot(codeVpp(used), sourceVpp(used), 'o', 'MarkerSize', 7, ...
    'LineWidth', 1.3, 'MarkerFaceColor', [0.10 0.45 0.75], ...
    'Color', [0.10 0.45 0.75]);
plot(codeVpp(~used), sourceVpp(~used), 'x', 'MarkerSize', 9, ...
    'LineWidth', 1.8, 'Color', [0.85 0.25 0.15]);
xLine = linspace(0, max(codeVpp) * 1.04, 300).';
plot(xLine, fit.slope * xLine + fit.intercept, '-', ...
    'LineWidth', 1.6, 'Color', [0.15 0.15 0.15]);
grid on;
box on;
xlabel('X2G Code_{pp} (code)');
ylabel('Source setting (V_{pp})');
title(sprintf(['AD9245 X2G calibration: %.6f uV/code, ', ...
    'R^2 = %.9f'], fit.slope * 1e6, fit.r2));
legend({'Primary fit points', 'Near-rail validation point', ...
    'Primary linear fit'}, 'Location', 'northwest');
...
title('Residuals; excluded point remains visible');
...
fprintf(fileId, 'AD9245 X2G scale calibration\n');
