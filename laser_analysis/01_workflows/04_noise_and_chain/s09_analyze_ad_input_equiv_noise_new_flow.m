%s09_analyze_ad_input_equiv_noise_new_flow - 折算 ADC 链路输入等效噪声
%   s09_analyze_ad_input_equiv_noise_new_flow 将 ADC-FPGA-DAC 总链路在
%   DAC 输出端测得的电压噪声折算到 ADC 外部输入参考面。
%
%   默认换算关系为：
%       scale = L_ADC/(K_out*G)
%       PSD_ADC_in(f) = PSD_Vout_total(f)*scale^2
%   K_out 是 DAC 输出 Vpp/Codepp 斜率，单位 V/code；L_ADC 是 ADC
%   外部输入 Vpp/Codepp 斜率，单位 V/code；G 是 FPGA 电压码值增益。
%   三者必须来自同一链路口径，不能把峰值刻度和 Vpp 刻度混用。
%
%   标定来源优先级由 useCalibrationWorkbook 和 useLatestFitSummary
%   控制。两者都关闭时使用 cfg.K_out/cfg.L_ADC；启用最新 summary
%   时读取 s07/s08 的已复核结果。截距不参与去均值噪声谱缩放。
%
%   正式默认 dacBaselineMode='none'，结果包含 DAC/Pico 本底。只有
%   本底与总链路的采样率、带宽、量程、终端和测量仪器条件匹配时，
%   才能改成 rawMat。可选相减关系为：
%       PSD_ADC_to_DACout = PSD_Vout_total - PSD_DAC_mid
%   只能相减 PSD，不能直接相减 ASD；负差值频点表示本底不确定性
%   已超过可分离信号，会保留为 NaN 并统计比例。
%
%   临时运行建议创建 s09_cfg_override，而不是反复改默认配置：
...
%
%   输出包括完整 PSD/ASD CSV、汇总和参数 CSV、MAT、源文件哈希及
%   对比图。指标频段超过 Nyquist 时只能报告暂不能判定。
%
%   See also s07_calibrate_ila_multi_sine, s08_calibrate_dac_multi_sine,
...
%localResolveGuiFiles - 在启用 GUI 时选择总链路和可选本底 MAT
if ~cfg.useGuiSelect
    return;
end
...
%localValidateConfig - 验证文件、增益、标定模式和 Welch 参数
...
%localResolveCalibration - 从工作簿中解析 ADC、DAC 斜率和链路比例
...
%localResolveDirectCalibration - 从最新 summary 或手动值解析链路比例
chainParam = struct();
chainParam.chainName = cfg.chainName;
chainParam.adcInterface = cfg.adcInterface;
chainParam.dacInterface = cfg.dacInterface;
...
%localCalibrationSource - 返回当前标定来源的可读说明
if cfg.useCalibrationWorkbook
...
%localFindCalibrationRow - 在标定表中查找唯一的器件/接口记录
...
%localComputeSpectrumFromMat - 从 Pico MAT 计算去均值 Welch PSD
...
%localLoadBaselineSpectrum - 加载并插值可选 DAC/Pico 本底 PSD
...
%localGetSampleRate - 按 Tinterval、fs、手动值顺序解析采样率
...
%localMakeWelchWindow - 按目标分辨率或分段数生成 Hann Welch 窗
...
%localMakeSummaryTable - 汇总各频段的两种本底口径和覆盖状态
...
%localBandStats - 计算单个频段的有限值 ASD/PSD 统计量
if hasData
...
function localWriteParameterTable(cfg, chainParam, totalSpec, baselineSpec, ...
        totalSha256, outPath)
%localWriteParameterTable - 导出配置、标定、频谱和源文件哈希
...
%localPlotAsdCompare - 保存未扣和可选扣本底的输入等效 ASD 图
...
%localPlotPsdCompare - 保存未扣和可选扣本底的输入等效 PSD 图
fig = localCreateFigure(cfg);
...
%localPlotSubtractionCheck - 保存 DAC 输出端 PSD 相减合理性检查图
fig = localCreateFigure(cfg);
...
%localCreateFigure - 按 showFigures 配置创建白色图窗
visibleState = 'off';
...
%localSaveFigure - 按指定分辨率导出图像并按需关闭图窗
...
%localApplyCfgOverride - 用工作区结构覆盖顶层配置字段
if isempty(override)
    return;
end
...
%localSha256 - 计算输入文件 SHA-256 以支持结果追溯
