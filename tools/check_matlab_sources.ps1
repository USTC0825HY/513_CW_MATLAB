...
$untrackedSet = [System.Collections.Generic.HashSet[string]]::new([StringComparer]::OrdinalIgnoreCase)
git -c safe.directory='*' -C $root ls-files --others --exclude-standard | ForEach-Object { [void]$untrackedSet.Add($_) }
$untracked = @($candidates | ForEach-Object {
  $rel = $_.FullName.Substring($root.Length + 1).Replace('\','/')
  if ($untrackedSet.Contains($rel)) { $rel }
})
