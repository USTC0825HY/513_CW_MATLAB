...
def make_doc():
    ...
    info_rows = [
        ...
        ["处理配置", "14 bit；signed；采样率配置 25 MHz；满量程峰值码 8192", "结果依据", "当前目录下各测试项目 results 汇总 CSV"],
    ...
    summary_rows = [
        ["AD9245", "采样率", "≥20 MSPS", "配置25 MHz；需外部实测", "X1G～X4G：25 MHz为处理配置值", "暂不能判定", "CSV无独立时标"],
    ...
    condition_rows = [
        ...
        ["采样率", "25 MHz", "处理配置值；不是AD9245采样时钟的独立实测值"],
    ...
    add_body(doc, "频率文件存在命名偏差，X3G 功率数据存在重复；ADC 实际采样时钟、1 Hz 噪声和温度记录未提供。25 MHz 为处理配置值。")
    ...
    follow_rows = [
        ...
        ["采样率", "25 MHz为处理配置值，无AD时钟实测证据", "用示波器或频率计测AD9245实际采样时钟；同时记录FPGA/ILA时钟、占空比和时钟来源", "确认≥20 MSPS判据", "高"],
    ]
